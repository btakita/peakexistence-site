...
+ dl_tree_({ ctx, _: sticky__dl_tree_props_ }, ()=>[
	...
	[`## ${mobile_hidden__span_([`How is `])}Ultrasound ${mobile_hidden__span_([`Used for `])}Vessel Screening${mobile_hidden__span_([`?`])}`, [
		...
		()=>[
			...
			[`### Peripheral Artery Disease (PAD)`, [
				...
				`- leg pain`,
				`- wounds`,
				`- an increased risk of amputation`].join(nl)],
			[`### Abdominal Aortic Aneurysm (AAA)`, [
				`Ultrasound can detect an abdominal aortic aneurysm, a potentially life-threatening bulge in the body's largest artery. Early detection enables monitoring & potential surgical intervention before the aneurysm ruptures${ref_s3_1_footnote__sup_(ctx)}.`].join(nl)],
			[`### Aneurysms`, [
				`The early detection of aneurysms. Particularly abdominal aortic aneurysms, through ultrasonic screening can be life-saving. Identifying these dilations in blood vessels early enables monitoring & management of the condition. Before it leads to vessel rupture, which can have fatal consequences${ref_s3_1_footnote__sup_(ctx)}.`].join(nl)],
			...
			[`### Chronic Venous Disease (CVD) Detection`, [
				...
				()=>[
					...
					[`#### Swelling`, [
						`Fluid buildup in the legs & ankles due to impaired blood flow.`].join(nl)],
					[`#### Skin changes`, [
						`Discoloration, dryness, or even ulcers can develop on the legs.`].join(nl)],
					[`#### Pain & discomfort`, [
	...
	[`## Who Should Consider Ultrasound Screening?`, [
		...
		`- Individuals experiencing symptoms like leg pain, unexplained swelling, or non-healing wounds.`].join(nl)],
	...
	[`## Conclusion`, [
		...
		`- aneurysms`,
		``,
		`As technology advances, the precision & utility of ultrasonic screening continue to improve. Underscoring its importance in maintaining cardiovascular health. Preventing the progression of chronic vessel diseases.`].join(nl)],
])
// @formatter:on
